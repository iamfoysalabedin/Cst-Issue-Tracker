...
import { Trash2, TrendingUp, TrendingDown, Minus, Building2, Calendar } from 'lucide-react';
import { ClientThreeMonthReport } from './ClientThreeMonthReport';

const MonthlyReports: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'clientReport' | 'monthlySummary'>('clientReport');
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-slate-900 p-2 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setActiveTab('clientReport')}
          ...
          >
            <Calendar size={16} />
            <span>Historical Monthly Summary ({entries.length})</span>
          </button>
        </div>
      </div>
                    ...
                    <tr key={e.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button 
                          ...
                          className="text-slate-400 hover:text-rose-500 transition-colors p-2 cursor-pointer"
                        >
                          <Trash2 size={16} />
                        </button>
                      </td>
